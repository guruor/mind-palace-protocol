...
The user asks a capable client to **install Mind Palace protocol** and provides
or confirms the common-memory destination. The agent performs two phases:

1. install or validate the protocol release in common memory;
2. configure the current client through its adapter.
...
client still needs its own thin adapter activation and receipt.
...
4. If the same version has another package identity or a component digest
   ...
   release.
5. If the candidate is newer, create it as `staged`. During `0.y.z`, assess
   compatibility explicitly. Activate only after required approval and client
   conformance, retaining the prior release for rollback.
...
After common memory passes, select the current host adapter. The adapter adds a
small bootstrap reference through a host-native additive mechanism. It must not
copy the protocol into an existing instruction file or replace user/system
instructions. The bootstrap tells the client when and how to resolve the active
common-memory installation; the release projection remains the behavior source.

Create or update one client receipt by stable client/installation identity and
run post-activation probes. Failure removes or disables only the managed adapter
entry and restores the previous client state.
